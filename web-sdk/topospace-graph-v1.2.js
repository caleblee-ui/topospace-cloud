...
class TopoSpaceGraphV12 extends HTMLElement {
  constructor(){
    super();
...
customElements.define("topospace-graph-v1-2",TopoSpaceGraphV12);
TopoSpaceGraphV12.prototype.pushFrame=function(frame){this.frames=this.frames||[];this.frames.push(frame);this.setGraph(frame.payload||frame);};
TopoSpaceGraphV12.prototype.setHistory=function(frames){this.frames=frames||[];if(this.frames.length)this.setGraph(this.frames[this.frames.length-1].payload||this.frames[this.frames.length-1]);};
window.TopoSpaceGraphV12SDK={mount(selector,data){const host=document.querySelector(selector),el=document.createElement("topospace-graph-v1-2");host.replaceChildren(el);el.setGraph(data);return el;}};
